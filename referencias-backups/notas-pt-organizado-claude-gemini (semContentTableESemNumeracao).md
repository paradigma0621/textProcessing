...
>
> Referências centrais do Gemini CLI:
> - Repositório: https://github.com/google-gemini/gemini-cli
> - Docs de comandos: https://google-gemini.github.io/gemini-cli/docs/cli/commands.html
...
## Carregando o VSCode a partir da pasta de uma sessão
...
## Selecionando arquivos de contexto
...
## Instalando e ativando a integração
...
## Recursos da integração (análogos aos do Claude)
...
# O arquivo CLAUDE.md → GEMINI.md (Claude)
...
## O que evitar (baixo valor ou arriscado)
...
# O arquivo CLAUDE.md → GEMINI.md (Gemini)
...
## Hierarquia (memória hierárquica)
...
## O que incluir / o que evitar / tratar como código
...
## Abordagem 1: O comando /config (UI interativa)
...
## Abordagem 2: Arquivos settings.json (configuração persistente)
...
### Qual configuração prevalece? (Ordem de prioridade)
...
### Uma estrutura típica de settings.json
...
## Definindo regras de permissão no Gemini
...
#### Casar com algo específico (allow/deny por comando de shell)
...
#### Exemplo do mundo real (o mesmo do Claude)
...
## Crie um Output Style personalizado no Claude Code

### O que é um Output Style personalizado?
...
### Estrutura de um Output Style personalizado
...
### Onde salvar seus arquivos de estilo
...
## 1. Via GEMINI.md (a forma mais simples e portável)
...
## 2. Via override do system prompt (`GEMINI_SYSTEM_MD`)
...
# IMPORTANTE — statelessness (Claude)
...
# IMPORTANTE — statelessness (Gemini)
...
## O que acontece quando o contexto enche?
...
## Diferença em relação ao `/rename` do Claude
...
## Como o Claude Code gerencia sessões entre branches do Git
...
# fork de sessão → conversational branching (Gemini)
...
## Sessões e branches do Git no Gemini
...
## Diferença em relação ao `/rewind`
...
# Os hábitos de sessão do desenvolvedor esperto (Claude)
...
# Os hábitos de sessão do desenvolvedor esperto (Gemini)
...
#### Auto Memory (MEMORY.md) — O caderno (escrito pelo CLAUDE)
...
### Como o Auto Memory funciona por baixo dos panos
...
### Onde o Claude armazena suas anotações?
...
### Dizendo ao Claude o que lembrar
...
## As três camadas de memória — o quadro completo
...
## Configurando o Auto Memory

### Como alternar (toggle)
...
### Diretório de memória personalizado
...
## ⚠️ Diferença central: NÃO há "Auto Memory" automática
...
## Mapeando as três camadas de memória
...
## Diretório de memória / como funciona
...
### Regras sempre carregadas (always-loaded)
...
### Regras com escopo de path (path-scoped)
...
### Rules vs CLAUDE.md — quando usar qual
...
## 1. GEMINI.md hierárquico por subdiretório (o análogo mais próximo do path-scoping)
...
## 2. Imports modulares no GEMINI.md
...
## 3. Extensões com regras/contexto
...
### Quando usar qual (mesma lógica do Claude)
...
## Construa sua própria skill

### Passos para criar uma skill personalizada
...
## Site de referência: code.claude.com/docs/en/skills (tem informações importantes)

Vale ver para aprofundar. Ver exemplos salvos na pasta `./skills` deste diretório.

## `./skills/feature-guide`: exemplo de como rodar uma skill em um sub-agente
...
## Reproduzindo a skill "analyze-issue"
...
## Diferenças em relação às Skills do Claude
...
## Agora, qual o propósito? Por que os hooks existem?
...
### Terceiro — consciência (awareness)
...
## Sem hooks, todas as três dependem do julgamento da IA.
...
## Exemplos de prompts que podem ser usados com Playwright (alguns com Google Chrome)
...
## Context7 MCP: o Claude recebe docs atualizadas
...
### Um comando. Docs sempre frescas para o Claude.
...
### Context7 no Gemini — docs sempre frescas
...
## Rodando prompts em um agendamento
...
## /loop — Rode um prompt em repetição, sem usar as mãos
...
## Intervalo inteligente (`/loop ...` sem tempo)
...
## O prompt de manutenção embutido
...
## 1. Headless mode + shell loop (o análogo mais direto)
...
## 2. Cron / agendador do SO (agendamento durável)
...
## 3. GitHub Actions (durável, no servidor)
...
## Memória persistente em Sub-Agents
...
## O que acontece quando a memória está habilitada?
...
## Como usar a memória de forma eficaz
...
## Delegação automática e explícita
...
## Memória persistente em sub-agentes
...
# Agent teams (equipes de agentes) (Claude)
...
# Agent teams (equipes de agentes) (Gemini)
...
## Claude Code para tarefas e atividades no GitHub
...
## Rode sessões paralelas do Claude Code com Git Worktrees

### Por que precisamos de worktrees?
...
## Integração com GitHub Actions (o bot @claude)
...
## Configurando a GitHub Action — Início rápido
...
## Plugin commit-commands → custom commands no Gemini
...
## Integração com GitHub Actions (o bot @gemini-cli)
...
## Como a Status Line funciona — JSON na entrada, texto na saída
...
## O caminho "eu nem escrevi um script"
...
## O caminho manual (`~/.claude/settings.json`)
...
### O arquivo statusline-command.sh
...
## Apêndice: tabela-resumo Claude ↔ Gemini
